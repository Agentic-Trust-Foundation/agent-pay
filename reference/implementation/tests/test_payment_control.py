...
from agent_pay.domain import Money, PaymentIntent, PaymentStatus
...
def service(provider=ProviderOutcome.SUCCEEDED):
    return PaymentService(
        SpendingPolicy(
            Decimal("500"),
            notify_above=Decimal("20"),
            approval_above=Decimal("100"),
        ),
        Budget(Decimal("1000")),
...
def test_success_consumes_budget():
    s = service()
    result = s.create_payment(intent(amount="50"))
...
def test_provider_timeout_is_unknown_not_failed():
    s = service(ProviderOutcome.UNKNOWN)
    result = s.create_payment(intent(amount="50"))
...
def test_idempotent_replay_returns_same_result():
    s = service()
    first = s.create_payment(intent(amount="50"))
    second = s.create_payment(intent(amount="50"))
    assert second == first
    assert s.budget.consumed == Decimal("50")
...
def test_idempotency_key_conflict_is_rejected():
    s = service()
    s.create_payment(intent(amount="50"))
...
def test_failed_provider_releases_reservation():
    s = service(ProviderOutcome.FAILED)
    result = s.create_payment(intent(amount="75"))
