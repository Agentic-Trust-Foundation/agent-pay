...
class ControlRepository:
    ...
    def evaluate_policy(self, *, account_id: UUID, policy_id: UUID | None,
        ...
        policy_id_value, version_id, version, rules = row
        intent = PaymentIntent(
            payment_id=str(payment_id), agent_id=str(agent_id),
            amount=Money(amount, currency), merchant_domain=merchant_domain,
            idempotency_key="",
        )
        decision = SpendingPolicy.from_rules(rules).evaluate(intent, category=category)
        self.conn.execute(
            "UPDATE payment_requests SET policy_version_id=%s WHERE id=%s",
            (version_id, payment_request_id),
        )
        ...
        if budget_id:
            row = self.conn.execute(
                """SELECT id FROM budgets
                   WHERE id=%s AND account_id=%s AND currency=%s AND status='ACTIVE'
                     AND (policy_id=%s OR policy_id IS NULL)""",
                (budget_id, account_id, currency, policy_id),
            ).fetchone()
            return row[0] if row else None
        rows = self.conn.execute(
            """SELECT id FROM budgets
               WHERE account_id=%s AND currency=%s AND status='ACTIVE'
                 AND (policy_id=%s OR policy_id IS NULL)
               ORDER BY created_at""",
            (account_id, currency, policy_id),
        ).fetchall()
    ...
    def create_approval(self, payment_request_id: UUID, *, reason: str | None = None) -> UUID:
        return self.conn.execute(
            ...
            (payment_request_id, reason),
        ).fetchone()[0]

    def get_approval(self, approval_id: UUID):
        return self.conn.execute(
            """SELECT id, payment_request_id, status, expires_at, approved_at, approved_by
    ...
    def payment_request(self, payment_request_id: UUID):
        return self.conn.execute(
            """SELECT pr.id, pr.account_id, pr.agent_id, pr.amount, pr.currency,
                      pr.purpose, pr.items, pr.policy_version_id, pr.budget_reservation_id,
